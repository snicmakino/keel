...
internal data class CliArgs(val socketPath: Path, val konancJar: Path, val konanHome: Path)
...
fun main(args: Array<String>) {
  val cli =
    parseArgs(args)
      .mapBoth(
        success = { it },
        failure = { err ->
          System.err.println("kolt-native-compiler-daemon: ${formatCliError(err)}")
          System.err.println(
            "usage: kolt-native-compiler-daemon --socket <path> --konanc-jar <path> --konan-home <path>"
          )
          exitProcess(64)
        },
      )
  ...
  val compiler =
    ReflectiveK2NativeCompiler.create(cli.konancJar)
      .mapBoth(
        success = { it },
        failure = { err ->
          System.err.println(
            "kolt-native-compiler-daemon: failed to initialise compiler: ${formatSetupError(err)}"
          )
          exitProcess(70)
        },
      )
  ...
  val server =
    DaemonServer(socketPath = cli.socketPath, compiler = compiler, config = DaemonConfig())
  val reason =
    server
      .serve()
      .mapBoth(
        success = { it },
        failure = { err ->
          System.err.println("kolt-native-compiler-daemon: serve failed: $err")
          exitProcess(71)
        },
      )
  System.err.println("kolt-native-compiler-daemon: exiting (${reason::class.simpleName})")
  exitProcess(0)
}
...
internal fun parseArgs(args: Array<String>): Result<CliArgs, CliError> {
  ...
  while (i < args.size) {
    when (args[i]) {
      "--socket" -> {
        socketPath = args.getOrNull(i + 1)
        i += 2
      }
      "--konanc-jar" -> {
        konancJar = args.getOrNull(i + 1)
        i += 2
      }
      "--konan-home" -> {
        konanHome = args.getOrNull(i + 1)
        i += 2
      }
...
private fun formatCliError(err: CliError): String =
  when (err) {
...
private fun formatSetupError(err: SetupError): String =
  when (err) {
    is SetupError.KonancJarNotFound -> "konanc jar not found at ${err.path}"
    is SetupError.KonancJarUnreadable ->
      "konanc jar at ${err.path} is unreadable: ${err.cause.message ?: err.cause.javaClass.name}"
