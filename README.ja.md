...
> v0.10.0 — 初期段階のプロジェクトです。破壊的変更の可能性があります。
...
- PATH 上の `kotlinc`（`kolt.toml` のバージョンと一致）
- PATH 上の `java`（JVM ターゲットの実行とテスト用）
...
> 以前は `keel` という名前でした。[keel.sh](https://keel.sh) および [Spinnaker keel](https://github.com/spinnaker/keel) との衝突を避けるため、v0.9.0 でリネームしました。
