...
> v0.19.0 — 初期段階のプロジェクトです。破壊的変更の可能性があります。
...
kolt remove <ga>       依存を kolt.toml から削除
...
`kolt remove <group:artifact>` で依存を削除します。 `[dependencies]` と `[test-dependencies]` の両方を自動検索し、 両方に同じ座標があれば両方から削除します。 `~/.kolt/cache/` の共有キャッシュは触らないので、 ディスク掃除は別途 `kolt cache clean` を実行してください。
...
contributor やインストーラー未対応 platform の利用者向け。事前に kolt のインストールが必要 (現状 linux-x64 のみ対応 — [Installation](#installation) 参照):
...
バイナリは `build/debug/kolt.kexe` に生成されます。PATH 上のディレクトリにコピーしてください:
