...
private constructor(
  ...
  private fun executeCompile(request: IcRequest): Result<IcResponse, IcError> {
    ...
    builder[BuildOperation.METRICS_COLLECTOR] =
      object : BuildMetricsCollector {
    ...
    return when (compilationResult) {
      ...
      CompilationResult.COMPILATION_ERROR -> {
        ...
        Err(
          IcError.CompilationFailed(
            messages = captured.ifEmpty { listOf("kotlinc reported $compilationResult") }
          )
        )
      }
      ...
      CompilationResult.COMPILER_INTERNAL_ERROR ->
        Err(IcError.InternalError(RuntimeException("kotlinc reported $compilationResult")))
    }
  }
