...
fun main(args: Array<String>) {
  val cli =
    parseArgs(args)
      .mapBoth(
        success = { it },
        failure = { err ->
          System.err.println("kolt-jvm-compiler-daemon: ${formatCliError(err)}")
          System.err.println(
            "usage: kolt-jvm-compiler-daemon --socket <path> --compiler-jars <classpath> --bta-impl-jars <classpath>"
          )
          exitProcess(64)
        },
      )
  ...
  val adapter =
    BtaIncrementalCompiler.create(
      ...
      .mapBoth(
        ...
        failure = { err ->
          System.err.println(
            "kolt-jvm-compiler-daemon: failed to initialise incremental compiler: " +
  ...
  val compiler = SelfHealingIncrementalCompiler(delegate = adapter, metrics = metrics)
  ...
  val server =
    DaemonServer(
      ...
      config = DaemonConfig(),
      preServeHook = { IcReaper.runDetached(cli.icRoot, KOLT_DAEMON_KOTLIN_VERSION, metrics) },
    )
  val reason =
    server
      .serve()
      .mapBoth(
        success = { it },
        failure = { err ->
          System.err.println("kolt-jvm-compiler-daemon: serve failed: $err")
          exitProcess(71)
        },
      )
  System.err.println("kolt-jvm-compiler-daemon: exiting (${reason::class.simpleName})")
  exitProcess(0)
}
...
internal fun parseArgs(args: Array<String>): Result<CliArgs, CliError> {
  ...
  while (i < args.size) {
    when (args[i]) {
      ...
      "--compiler-jars" -> {
        compilerJars = args.getOrNull(i + 1)
        i += 2
      }
      "--bta-impl-jars" -> {
        btaImplJars = args.getOrNull(i + 1)
        i += 2
      }
      "--ic-root" -> {
        icRoot = args.getOrNull(i + 1)
        i += 2
      }
      "--plugin-jars" -> {
...
internal fun parsePluginJars(raw: String?): Result<Map<String, List<Path>>, CliError> {
  ...
  for (entry in raw.split(';')) {
    ...
    val cp =
      entry.substring(eq + 1).split(File.pathSeparator).filter { it.isNotBlank() }.map(Path::of)
