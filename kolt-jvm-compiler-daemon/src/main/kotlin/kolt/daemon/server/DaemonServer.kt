...
class DaemonServer(
  ...
  private fun writeProtocolError(output: OutputStream, stderr: String) {
    FrameCodec.writeFrame(
      output,
      Message.CompileResult(exitCode = 2, diagnostics = emptyList(), stdout = "", stderr = stderr),
    )
  }
  ...
  private fun icErrorToReply(error: IcError): Message.CompileResult =
    when (error) {
