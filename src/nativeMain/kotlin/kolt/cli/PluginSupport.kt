...
private fun defaultPluginFetcherDeps(): PluginFetcherDeps =
  object : PluginFetcherDeps {
...
private fun formatDownloadError(err: DownloadError): String =
  when (err) {
...
internal fun resolvePluginArgs(
  ...
  resolveEnabledPluginJarsResult(config, paths, operationalExitCode).map {
    it.values.map { path -> "-Xplugin=$path" }
  }
...
internal fun resolvePluginJarsMap(
  ...
  resolveEnabledPluginJarsResult(config, paths, operationalExitCode).map {
    it.mapValues { (_, path) -> listOf(path) }
  }
