...
internal data class HomeBreakdown(
  ...
  val totalBytes: Long
    get() = cacheBytes + toolchainsBytes + daemonBytes + toolsBytes
}
...
internal data class JdkInfo(val version: String?, val path: String, val source: String? = null)
...
private fun formatInfoDefault(snap: InfoSnapshot): String =
  buildString {
      ...
      snap.kotlin?.let { appendLine(labeled("kotlin", "${it.version} (${it.mode}, ${it.path})")) }
      snap.jdk?.let { appendLine(labeled("jdk", "${it.version} (${it.path})")) }
...
private fun formatInfoVerbose(snap: InfoSnapshot): String =
  buildString {
      appendLine(
        labeled(
          "kolt",
          "v${snap.koltVersion} (${snap.koltPath ?: UNKNOWN_DISPLAY})",
          VERBOSE_LABEL_WIDTH,
        )
      )
      appendLine()
      ...
      snap.koltHomeBreakdown?.let { b ->
        ...
        appendLine(
          subLabeled("toolchains", "${b.toolchainsPath} (${formatBytes(b.toolchainsBytes)})")
        )
      ...
      snap.jdk?.let { j ->
        ...
        val headerValue =
          listOf(versionPart, sourcePart).filter { it.isNotEmpty() }.joinToString(" ")
      ...
      if (snap.project != null) {
        appendLine()
        appendLine(
          labeled("project", "${snap.project.name} v${snap.project.version}", VERBOSE_LABEL_WIDTH)
        )
...
internal fun doInfo(args: List<String>): Result<Unit, Int> {
  val opts =
    parseInfoArgs(args).getOrElse { msg ->
      eprintln(msg)
...
internal fun loadProjectForInfo(): ProjectLoad {
  ...
  val toml =
    readFileAsString(KOLT_TOML).getOrElse { err ->
...
@Serializable
private data class InfoJdkJson(val version: String? = null, val path: String, val source: String)
...
internal fun formatInfoJson(snap: InfoSnapshot): String {
  ...
  val jdk =
    snap.jdk?.let { j ->
      InfoJdkJson(version = j.version, path = j.path, source = j.source ?: "managed")
    }
  val project =
    snap.project?.let { p ->
      InfoProjectJson(
