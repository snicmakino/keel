...
internal fun validateToolchainRemoveArgs(args: List<String>): Result<ToolchainRemoveArgs, String> {
  ...
  if (name !in KNOWN_TOOLCHAINS) {
    return Err(
      "error: unknown toolchain '$name' (available: ${KNOWN_TOOLCHAINS.joinToString(", ")})"
    )
  }
  return Ok(ToolchainRemoveArgs(name, args[1]))
}

internal fun resolveToolchainPathForRemove(
  name: String,
  version: String,
  paths: KoltPaths,
): String? {
...
private fun doToolchainRemove(args: List<String>): Result<Unit, Int> {
  val parsed =
    validateToolchainRemoveArgs(args).getOrElse { error ->
      eprintln(error)
      return Err(EXIT_CONFIG_ERROR)
    }
