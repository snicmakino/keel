...
internal fun reapStaleDaemons(daemonBaseDir: String): ReapResult {
  ...
  val projectDirs =
    listSubdirectories(daemonBaseDir).getOrElse {
  ...
  for (projectDir in projectDirs) {
    ...
    for (versionDir in versionDirs) {
      ...
      // (`jvm-compiler-daemon.sock`, `jvm-compiler-daemon-noplugins.sock`,
      // `jvm-compiler-daemon-<8hex>.sock`),
