...
internal fun stopProjectDaemons(
  ...
  var stopped = 0
  val versionDirs =
    listSubdirectories(projectDir).getOrElse {
      return stopped
    }
...
internal fun isJvmDaemonSocket(name: String): Boolean =
  name == "jvm-compiler-daemon.sock" ||
    (name.startsWith("jvm-compiler-daemon-") &&
      name.endsWith(".sock") &&
      name.length > "jvm-compiler-daemon-.sock".length)

internal fun isNativeDaemonSocket(name: String): Boolean = name == "native-compiler-daemon.sock"
...
private fun stopAllDaemons(daemonBaseDir: String) {
  ...
  val projectDirs =
    infraListSubdirectories(daemonBaseDir).getOrElse {
      println("no daemons running")
      return
    }
  var stopped = 0
...
private fun sendShutdown(socketPath: String): Boolean {
  val socket =
    UnixSocket.connect(socketPath).getOrElse {
      return false
    }
...
private fun printDaemonUsage() {
  ...
  eprintln("commands:")
  eprintln(
    "  stop       Stop this project's compiler daemons (JVM and native; --all for all projects)"
  )
