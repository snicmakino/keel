...
// path.
data class NativeCompileOutcome(val stderr: String)
...
sealed interface NativeCompileError {
  ...
  // fail the same way.
  data class CompilationFailed(val exitCode: Int, val stderr: String) : NativeCompileError
...
fun isNativeFallbackEligible(error: NativeCompileError): Boolean =
  when (error) {
...
fun formatNativeCompileError(error: NativeCompileError, context: String): String =
  when (error) {
    is NativeCompileError.CompilationFailed ->
      "error: $context failed with exit code ${error.exitCode}"
    ...
    is NativeCompileError.BackendUnavailable.PopenFailed ->
      "error: failed to start $context process"
