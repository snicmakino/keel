...
object FrameCodec {
  ...
  fun writeFrame(socket: UnixSocket, message: Message): Result<Unit, FrameError> {
    ...
    socket.sendAll(header).getOrElse {
      return Err(FrameError.Transport(it))
    }
    socket.sendAll(body).getOrElse {
      return Err(FrameError.Transport(it))
    }
    return Ok(Unit)
  }
  ...
  fun readFrame(socket: UnixSocket): Result<Message, FrameError> {
    val header =
      socket.recvExact(4).getOrElse { err ->
    ...
    val body =
      socket.recvExact(len).getOrElse { err ->
        return Err(
          when (err) {
  ...
  private fun headerTransportError(err: UnixSocketError): FrameError =
    when (err) {
      is UnixSocketError.UnexpectedEof ->
