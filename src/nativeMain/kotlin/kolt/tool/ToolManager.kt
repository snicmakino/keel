...
internal val CONSOLE_LAUNCHER_SPEC =
  ToolSpec(
    ...
    return Err(
      when (error) {
        is DownloadError.HttpFailed ->
          "failed to download ${spec.artifact} (HTTP ${error.statusCode})"
        is DownloadError.WriteFailed -> "could not write $path"
        is DownloadError.NetworkError ->
          "network error downloading ${spec.artifact}: ${error.message}"
      }
    )
  }
  return Ok(path)
