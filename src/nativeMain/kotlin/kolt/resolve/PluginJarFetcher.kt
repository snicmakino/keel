...
sealed interface PluginFetchError {
  ...
  data class DownloadFailed(val alias: String, val url: String, val cause: DownloadError) :
    PluginFetchError
...
private val ALIAS_TO_ARTIFACT: Map<String, String> =
  mapOf(
...
/**
 * Ensures that the compiler plugin jar for [alias] at [kotlinVersion] is present under [cacheBase]
 * and returns its absolute path.
 *
 * The fetcher uses a TOFU sha256 stamp next to the cached jar: on first download the local sha256
 * is computed and written to `<jar>.sha256`, and on subsequent calls the jar is re-hashed and
 * compared against the stamp. A missing or mismatched stamp triggers a re-download (self- healing
 * against legacy cache contents or bit rot). Maven Central is pinned; see the module doc for the
 * rationale.
 */
fun fetchPluginJar(
  alias: String,
  ...
  val artifact = ALIAS_TO_ARTIFACT[alias] ?: return Err(PluginFetchError.UnknownPlugin(alias))
  ...
  val url =
    "$MAVEN_CENTRAL_BASE_URL/org/jetbrains/kotlin/$artifact/$kotlinVersion/$artifact-$kotlinVersion.jar"
  ...
  if (deps.fileExists(jarPath) && deps.fileExists(stampPath)) {
    val storedStamp =
      deps
        ...
        ?.trim()
    val actualHash =
      deps.computeSha256(jarPath).getOrElse { err ->
  ...
  val freshHash =
    deps.computeSha256(jarPath).getOrElse {
...
fun fetchEnabledPluginJars(
  ...
  for ((alias, enabled) in plugins) {
    if (!enabled) continue
    val path =
      fetchPluginJar(alias, kotlinVersion, cacheBase, deps).getOrElse {
        return Err(it)
      }
    out[alias] = path
  }
