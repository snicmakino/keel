...
data class PomParent(val groupId: String, val artifactId: String, val version: String)
...
fun parsePom(xml: String): Result<PomInfo, PomParseError> {
  ...
  val projectArtifactId =
    root.childText("artifactId") ?: return Err(PomParseError("Missing <artifactId>"))
  ...
  val depMgmt =
    root
      ...
      ?.map { parseDependencyElement(it, interpolationMap) } ?: emptyList()
  ...
  return Ok(
    PomInfo(parent, projectGroupId, projectArtifactId, projectVersion, properties, depMgmt, deps)
  )
}
...
private fun interpolate(value: String, properties: Map<String, String>): String {
  ...
  repeat(10) {
    val next =
      PROPERTY_REGEX.replace(result) { match ->
...
private class XmlElement(val name: String, val text: String?, val children: List<XmlElement>) {
    ...
    if (nextOpen != -1 && nextOpen < nextClose) {
      ...
      if (
        afterOpen < xml.length &&
          (xml[afterOpen] == '>' || xml[afterOpen] == ' ' || xml[afterOpen] == '/')
      ) {
