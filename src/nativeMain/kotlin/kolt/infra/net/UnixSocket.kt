...
class UnixSocket internal constructor(private val fd: Int) : AutoCloseable {
  ...
  fun recvExact(n: Int): Result<ByteArray, UnixSocketError> {
    if (n < 0) {
      return Err(UnixSocketError.InvalidArgument("recvExact length must be non-negative, was $n"))
    }
    val buf = ByteArray(n)
  ...
  companion object {
    ...
    fun connect(path: String): Result<UnixSocket, UnixSocketError> {
      ...
      return memScoped {
        ...
        var rc: Int
        while (true) {
          rc = platform.posix.connect(fd, addr.ptr.reinterpret<sockaddr>(), addrLen)
          if (rc == 0) break
          val e = errno
          if (e != EINTR) {
            platform.posix.close(fd)
            return@memScoped Err(UnixSocketError.ConnectFailed(path, e, strerrorMessage(e)))
          }
        }
        Ok(UnixSocket(fd))
      }
...
sealed interface UnixSocketError {
  data class ConnectFailed(val path: String, val errno: Int, val message: String) : UnixSocketError

  data class SendFailed(val errno: Int, val message: String) : UnixSocketError

  data class RecvFailed(val errno: Int, val message: String) : UnixSocketError

  data class UnexpectedEof(val received: Int, val expected: Int) : UnixSocketError

  data class ShutdownFailed(val errno: Int, val message: String) : UnixSocketError
