    ...
    UnixEchoServer.start()
      ...
      .use { server ->
        UnixSocket.connect(server.socketPath)
          .getOrElse { fail("connect failed: $it") }
          .use { client ->
            FrameCodec.writeFrame(client, message).getOrElse { fail("writeFrame failed: $it") }
            client.shutdownWrite().getOrElse { fail("shutdownWrite failed: $it") }
            return FrameCodec.readFrame(client).getOrElse { fail("readFrame failed: $it") }
          }
      }
  }

  private fun withReadFrame(server: UnixEchoServer, prime: (UnixSocket) -> Unit): FrameError {
    UnixSocket.connect(server.socketPath)
      .getOrElse { fail("connect failed: $it") }
      .use { client ->
        prime(client)
