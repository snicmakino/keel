...
class InotifyWatcherTest {
  ...
  fun detectsFileCreation() {
    ...
    try {
      val watcher = InotifyWatcher.create().getOrElse { fail("create failed: $it") }
      try {
        watcher.addWatch(dir, (IN_CREATE or IN_MODIFY).toUInt()).getOrElse {
          fail("addWatch failed: $it")
        }

        writeFileAsString("$dir/hello.kt", "fun main() {}").getOrElse { fail("write failed") }

        val events =
          watcher.pollEvents(timeoutMs = 3000).getOrElse { fail("pollEvents failed: $it") }
  ...
  fun addWatchRecursiveCoversSubdirs() {
    ...
    try {
      ...
      try {
        ...
        writeFileAsString("$dir/sub/deep/Nested.kt", "class Nested").getOrElse {
          fail("write failed")
        }
  ...
  fun pollReturnsEmptyOnTimeout() {
    ...
    try {
      val watcher = InotifyWatcher.create().getOrElse { fail("create failed: $it") }
      try {
        watcher.addWatch(dir, IN_MODIFY.toUInt()).getOrElse { fail("addWatch failed: $it") }
        val events = watcher.pollEvents(timeoutMs = 50).getOrElse { fail("pollEvents failed: $it") }
