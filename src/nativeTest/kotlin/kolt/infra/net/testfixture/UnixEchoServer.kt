...
private constructor(
  ...
  companion object {
    ...
    @OptIn(ExperimentalForeignApi::class)
    fun start(handler: (ByteArray) -> ByteArray = { it }): Result<UnixEchoServer, StartError> {
      ...
      val workerState = EchoWorkerState(listenFd = fd, handler = handler, shutdown = AtomicInt(0))
    ...
    @OptIn(ExperimentalForeignApi::class)
    private fun bindAndListen(fd: Int, path: String, pathBytes: ByteArray): StartError? =
      memScoped {
