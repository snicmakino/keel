...
class FallbackNativeCompilerBackendTest {
  ...
  fun primaryBackendUnavailableRetriesOnFallback() {
    val primary =
      FakeBackend(reply = Err(NativeCompileError.BackendUnavailable.Other("daemon unreachable")))
  ...
  fun primaryInternalMisuseFallsBack() {
    val primary =
      FakeBackend(reply = Err(NativeCompileError.InternalMisuse("socket path too long")))
    val fallback = FakeBackend(reply = Ok(NativeCompileOutcome(stderr = "")))
  ...
  fun bothBackendsFailReturnsFallbackError() {
    ...
    val fallback =
      FakeBackend(reply = Err(NativeCompileError.CompilationFailed(exitCode = 1, stderr = "")))
  ...
  private class FakeBackend(private val reply: Result<NativeCompileOutcome, NativeCompileError>) :
