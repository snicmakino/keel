...
class RenderCompilationFailureTest {
  ...
  fun rendersStructuredDiagnosticsAsKotlincStylePositionedLines() {
    ...
    assertEquals("/tmp/Main.kt:3:5: error: expected ';'", renderCompilationFailure(error))
  }
  ...
  fun rendersMultipleDiagnosticsInOrder() {
    ...
    assertEquals(
      listOf("/a.kt:1:1: error: first", "/b.kt:2:2: warning: second", "/c.kt:3:3: info: third"),
      rendered.split("\n"),
    )
  }
  ...
  fun appendsPlainStderrAfterStructuredDiagnostics() {
    val error =
      CompileError.CompilationFailed(
        ...
        diagnostics = listOf(Diagnostic(Severity.Error, "/a.kt", 1, 1, "boom")),
      )
    val rendered = renderCompilationFailure(error)
    assertEquals("/a.kt:1:1: error: boom\n\tat foo.Bar.baz(Bar.kt:42)", rendered)
  }

  @Test
  fun subprocessPathProducesEmptyBody() {
    val error = CompileError.CompilationFailed(exitCode = 1, stdout = "", stderr = "")
  ...
  fun fileNullWithLineColumnStillCollapsesToNoLocation() {
    ...
    val error =
      CompileError.CompilationFailed(
        ...
        stderr = "",
        diagnostics =
          listOf(Diagnostic(Severity.Error, file = null, line = 5, column = 3, message = "rogue")),
      )
  ...
  fun rendersStderrOnlyWhenDiagnosticsAreEmpty() {
    val error =
      CompileError.CompilationFailed(exitCode = 1, stdout = "", stderr = "kotlinc: broken")
  ...
  fun trailingNewlineInStderrIsStripped() {
    val error =
      CompileError.CompilationFailed(exitCode = 1, stdout = "", stderr = "line 1\nline 2\n")
