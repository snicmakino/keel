...
class EnsureBootstrapJavaBinTest {
  ...
  fun installFailureSurfacesAsBootstrapJdkError() {
    ...
    val result =
      ensureBootstrapJavaBin(paths = paths, resolve = { null }, install = { _, _ -> Err(cause) })
  ...
  fun postInstallProbeFailureSurfacesAsBootstrapJdkError() {
    val result =
      ensureBootstrapJavaBin(paths = paths, resolve = { null }, install = { _, _ -> Ok(Unit) })

    val err = assertNotNull(result.getError())
    assertEquals(expectedInstallDir, err.jdkInstallDir)
    assertEquals("java binary not found at $expectedJavaBin after installation", err.cause.message)
  }
}
