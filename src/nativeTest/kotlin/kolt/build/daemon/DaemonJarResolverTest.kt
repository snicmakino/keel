...
class DaemonJarResolverTest {
  ...
  fun devFallbackResolvesFromBinaryInsideBuildBinLinuxX64() {
    ...
    val devJar =
      "/home/alice/src/kolt/kolt-jvm-compiler-daemon/build/libs/kolt-jvm-compiler-daemon-all.jar"
    val result =
      resolveDaemonJarPure(envValue = null, selfExePath = kolt, fileExists = exists(devJar))
    val resolved = assertIs<DaemonJarResolution.Resolved>(result)
  ...
  fun noSelfExeAndNoEnvReturnsNotFound() {
    val result = resolveDaemonJarPure(envValue = null, selfExePath = null, fileExists = { true })
