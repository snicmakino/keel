...
class RunnerTest {
  ...
  fun runCommandWithEmptyAppArgs() {
    val cmd =
      runCommand(testConfig(), main = "com.example.main", classpath = null, appArgs = emptyList())
    assertEquals(listOf("java", "-cp", "build/classes", "com.example.MainKt"), cmd.args)
  }
  ...
  fun runCommandWithManagedJavaPathUsesItAsJava() {
    ...
    assertEquals(listOf(managedJavaBin, "-cp", "build/classes", "com.example.MainKt"), cmd.args)
  }
