...
class RuntimeClasspathManifestTest {
  ...
  fun writeRuntimeClasspathManifestSortsByFileNameAndExcludesSelfJar() {
    ...
    val content =
      readFileAsString(outputRuntimeClasspathPath(config)).getOrElse { error("read failed: $it") }
  ...
  fun writeRuntimeClasspathManifestUsesLfWithoutTrailingNewline() {
    ...
    val jars =
      listOf(
        ResolvedJar(cachePath = "/cache/a/a/1/a-1.jar", groupArtifactVersion = "a:a:1"),
        ResolvedJar(cachePath = "/cache/b/b/1/b-1.jar", groupArtifactVersion = "b:b:1"),
      )
