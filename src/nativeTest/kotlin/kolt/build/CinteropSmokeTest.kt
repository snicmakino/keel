...
class CinteropSmokeTest {
  ...
  private val curlIncludeCandidates =
    listOf("/usr/include/x86_64-linux-gnu", "/usr/include", "/usr/local/include")

  private val curlLibCandidates =
    listOf("/usr/lib/x86_64-linux-gnu", "/usr/lib64", "/usr/lib", "/usr/local/lib")
  ...
  fun libcurlCinteropPipelineProducesRunningExecutable() {
    ...
    if (curlLibDir == null) {
      println("SKIP: libcurl.so not found in any of " + curlLibCandidates.joinToString())
      return
    }
    ...
    try {
      ...
      assertNotNull(cinteropResult.get(), "cinterop failed to generate klib: $cinteropResult")
      ...
      val stage1Result =
        executeCommand(
          listOf(konancBin, mainFile, "-p", "library", "-nopack", "-l", klibFile, "-o", appKlibBase)
        )
      assertNotNull(stage1Result.get(), "konanc stage 1 (compile to klib) failed: $stage1Result")
      ...
      assertNotNull(stage2Result.get(), "konanc stage 2 (link to kexe) failed: $stage2Result")
