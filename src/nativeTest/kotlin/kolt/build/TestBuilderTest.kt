...
class TestBuilderTest {
  ...
  fun testBuildCommandWithClasspath() {
    ...
    assertEquals(
      listOf(
        "kotlinc",
        "-cp",
        "build/classes:/cache/dep.jar:/cache/junit.jar",
        "test",
        "-jvm-target",
        "17",
        "-d",
  ...
  fun testBuildCommandDifferentJvmTarget() {
    val cmd = testBuildCommand(config = testConfig(jvmTarget = "21"), classesDir = "build/classes")
  ...
  fun testBuildCommandOutputPathIsTestClassesDir() {
    val cmd =
      testBuildCommand(config = testConfig(name = "hello-world"), classesDir = "build/classes")
  ...
  fun testBuildCommandWithPluginArgs() {
    ...
    val cmd =
      testBuildCommand(config = testConfig(), classesDir = "build/classes", pluginArgs = pluginArgs)
  ...
  fun testBuildCommandWithNullKotlincPathDefaultsToSystemKotlinc() {
    val cmd =
      testBuildCommand(config = testConfig(), classesDir = "build/classes", kotlincPath = null)
