...
private class FakeFetcherDeps(
  ...
  val downloadResult: (url: String, dest: String) -> Result<Unit, DownloadError> = { _, _ ->
    Ok(Unit)
  },
) : PluginFetcherDeps {
...
class PluginJarFetcherTest {
  ...
  fun coldCacheDownloadsComputesHashAndWritesStamp() {
    val deps = FakeFetcherDeps(sha256Results = mutableMapOf(serializationJar to "hash-abc"))
  ...
  fun cacheHitWithHashFailureWarnsBeforeReDownload() {
    ...
    assertEquals(
      1,
      deps.downloadCalls.size,
      "must attempt re-download after cache-hit hash failure",
    )
    assertTrue(
  ...
  fun downloadFailurePropagatesAsPluginFetchError() {
    val deps =
      FakeFetcherDeps(downloadResult = { url, _ -> Err(DownloadError.HttpFailed(url, 500)) })
  ...
  fun mkdirRunsOnColdCachePath() {
    val deps = FakeFetcherDeps(sha256Results = mutableMapOf(serializationJar to "h"))

    fetchPluginJar("serialization", kotlinVersion, cacheBase, deps)

    assertEquals(1, deps.mkdirCalls.size)
    assertTrue(
      deps.mkdirCalls.single().endsWith("kotlin-serialization-compiler-plugin/2.3.20"),
