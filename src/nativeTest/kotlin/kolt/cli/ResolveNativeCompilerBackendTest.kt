...
class ResolveNativeCompilerBackendTest {
    ...
    val backend =
      resolveNativeCompilerBackend(
        ...
        preconditionResolver = { _, _, _ ->
          error("must not probe preconditions when useDaemon=false")
        },
    ...
    val backend =
      resolveNativeCompilerBackend(
        ...
        daemonBackendFactory = {
          error("must not construct daemon backend when precondition failed")
        },
        warningSink = { warnings.add(it) },
      )
  ...
  private fun minimalConfig(kotlincVersion: String) =
    KoltConfig(
      ...
      build = BuildSection(target = "linuxX64", main = "itMainKt", sources = emptyList()),
    )
}
