...
class PluginSupportTest {
  ...
  fun resolvePluginArgsNoPluginsReturnsEmptyWithoutTouchingTheNetwork() {
    ...
    val result =
      assertNotNull(resolvePluginArgs(config, bogusPaths, operationalExitCode = 999).get())
  ...
  fun resolvePluginArgsAllDisabledReturnsEmptyWithoutTouchingTheNetwork() {
    val config = testConfig(plugins = mapOf("serialization" to false, "allopen" to false))
  ...
  fun resolvePluginJarsMapNoPluginsReturnsEmptyMap() {
    ...
    val result =
      assertNotNull(resolvePluginJarsMap(config, bogusPaths, operationalExitCode = 999).get())
    ...
    val exit =
      mapPluginFetchErrorToExit(
        PluginFetchError.DownloadFailed(
          alias = "serialization",
          url =
            "https://repo1.maven.org/maven2/.../kotlin-serialization-compiler-plugin-99.0.0.jar",
          cause =
