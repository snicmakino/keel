  ...
  fun enumeratesBothJvmAndNativeSocketsUnderEachVersionDir() {
    ...
    val fs =
      FakeFs(
        ...
        files =
          mapOf(
            "$projectDir/2.3.20" to
              listOf("jvm-compiler-daemon.sock", "native-compiler-daemon.sock")
          ),
      )
    ...
    val fs =
      FakeFs(
        ...
        files =
          mapOf(
            "$projectDir/2.3.20" to
              listOf("jvm-compiler-daemon-abcd1234.sock", "jvm-compiler-daemon-noplugins.sock")
          ),
      )
    val jvmSockets = mutableListOf<String>()
  ...
  fun nativeDaemonSockIsNotMatchedByJvmEnumeration() {
    ...
    val stopped =
      stopProjectDaemons(
        ...
        listFiles = fs::listFiles,
        sendJvmShutdown = {
          error("must not send — `native-compiler-daemon.sock` is not a JVM daemon")
        },
        sendNativeShutdown = {
          nativeSockets += it
          true
