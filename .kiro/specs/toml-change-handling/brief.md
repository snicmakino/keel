...
- `kolt add` は full sync (rewrite + resolve + lockfile + JAR cache) で完結する
...
- `kolt remove` が存在しない (add だけある非対称)
...
- **`kolt {add, fetch, update}` の挙動**: add は eager full sync、 fetch は re-read + resolve、 update は JAR cache 削除 + force resolve。 一貫した「edit → resolve → lockfile」モデル
- **`kolt remove` は存在しない** (`DependencyCommands.kt:279` は `doTree()` の終端)
...
- `kolt remove` は別 issue で follow-up (本 spec scope 外)
...
[watch]   ⚠ [dependencies] changed. Run `kolt fetch` to apply.
...
- **Q1=B + γ2**: per-invocation 経路は完全 eager に揃える (現状そうなっているのを ADR で固定)。 hand-edit は次の build/test/run で同じ post-state に収束。 `kolt remove` は対称性確保のため別 issue として切り出し
...
- **Downstream**:
  - #297 Q6: `kolt remove` follow-up issue (γ2)
  - 将来の config schema 拡張 (`[publish]` 等) は本 matrix を継承
  - IDE/LSP 統合は「明示的な `kolt fetch` を呼ぶ」契約になる
