...
**Decision notes**:
- 配置は **`copy` を default**（per-scope path に fresh inode を作る）。few-KB ファイルの copy は wall-time 的に無視できる規模、かつ BTA が pre-placed file を in-place 上書き (FileOutputStream 直書き) しても cache file の inode が共有されないので汚染リスクなし
- `hardlink` 最適化は spike T1 で BTA の write 実装（in-place vs tmp+atomic-rename）を確認した上で **opt-in 化を後続検討**。default は copy で安全側に倒す
...
| OQ-3 | hardlink を opt-in 化する場合の BTA write 実装 (in-place vs tmp+rename) | spike T1 §1 で `truss` / `strace` または `Files.getAttribute` 経由 inode 変化観測で確認、結果次第で copy/hardlink default を最終決定 |
