...
### コンパイラデーモン (`kolt-jvm-compiler-daemon/`)
...
### ネイティブコンパイラデーモン (`kolt-native-compiler-daemon/`)
...
daemon classloader (独立、kolt-native-compiler-daemon のコード + kotlin-result を保持)
```
...
- **ソケット**：`~/.kolt/daemon/<projectHash>/<kotlinVersion>/jvm-compiler-daemon.sock`（JVM）と `.../native-compiler-daemon.sock`（ネイティブ）。同じバージョンディレクトリに並置することで、`kolt daemon stop` を単一の列挙パスで済ませます（ADR 0020 §2、ADR 0024、ADR 0026 §3）。
...
`kolt.toml` 編集時の挙動は 3 値の `SectionAction` 分類（`AutoReload` / `NotifyOnly` / `NoOp`）と `kolt.config.ChangeMatrix` のセクション単位マトリクスで管理されます。各 CLI コマンド（`kolt build`、`kolt test`、`kolt run`、`kolt {add, fetch, update}`）は呼び出しごとに `kolt.toml` を fresh に読み直すので、編集は次回呼び出しで反映されます。watch モードは検知した変更を同じマトリクスで分類してディスパッチします：auto-reload セクション（`[build] sources`、`[run.sys_props]` 等）は静かに反映、notify-only セクション（`[dependencies]`、`[kotlin] compiler` 等）は stderr に通知を出してユーザーの明示操作を要求、no-op セクション（`[fmt]`）は無視。1 つの debounce window に auto-reload と notify-only が混在した場合は notify-only が prevail し、auto-reload は推奨アクション実行後の watch 再起動まで保留されます。詳細は [ADR 0033](adr/0033-kolt-toml-change-handling-model.md) を参照。
...
- [0033](adr/0033-kolt-toml-change-handling-model.md) — kolt.toml 設定変更ハンドリングモデル（SectionAction 分類 + per-invocation/watch マトリクス）
