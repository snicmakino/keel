...
- 2026-05-03 Warm `kolt test` with no source or test changes consistently runs ~15s across 4 back-to-back runs (junit-platform-console-standalone already cached at `~/.kolt/tools/`, daemon hot). The earlier reading of ~10.7s was a single observation; ~15s is the steady value on this hardware. Cost is dominated by the JUnit Console Launcher JVM cold start + test compile that fires unconditionally (note `compiling tests...` line — tests recompile even on no-source-change). Worth measuring whether a persistent test-runner JVM (parallel to the compile daemon) would amortize this for tight TDD loops → watching.
...
- 2026-05-03 `kolt remove <group:artifact>` leaves an empty `[dependencies]` table header in `kolt.toml`. Cosmetic only, but makes the file read as "still has dependencies" at a glance and looks unfinished in PR diffs → #360.
- 2026-05-03 Unknown subcommand (`kolt foobar`) exits 1; the README exit-code table reserves 127 for "Command not found". Either the table is shell-only (kolt binary absent) and should say so, or unknown-subcommand should map to 127. Doc/impl alignment either way → #361.
- 2026-05-03 Generated `.gitignore` contains only `build/`; first-time IntelliJ users will end up checking in `.idea/` and `*.iml`. Adding the standard JetBrains + OS-junk lines (`.idea/`, `*.iml`, `.DS_Store`) to the scaffold is a one-line UX win → #362.
