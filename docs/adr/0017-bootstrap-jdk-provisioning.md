...
`DaemonCompilerBackend` (ADR 0016) spawns a helper JVM with `<java> -jar kolt-compiler-daemon-all.jar --socket <path> --compiler-jars <cp>`. Before #14 Phase A, `kolt build` only needed a JDK for the final run step; compilation shelled out to `kotlinc`, which already bundled its own JRE. The daemon changes that: the daemon is a long-lived process kolt itself spawns, and a crash or refusal to start manifests as "slow builds" with no obvious root cause.
